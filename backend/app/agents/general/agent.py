...
- **积分兑换**：用户要兑换积分 → 使用 loyalty_points_redeem 工具
- **优惠券**：查询可用优惠券、使用规则 → 使用 customer_coupon_lookup 工具
- **使用优惠券**：用户要用优惠券 → 使用 coupon_apply 工具
- **产品推荐**：根据需求推荐合适的产品 → 使用 product_recommendation 工具
- **客户反馈**：查询反馈记录 → 使用 customer_feedback_lookup 工具
- **提交反馈**：用户要提交反馈 → 使用 customer_feedback_submit 工具
- **公司信息**：查询公司政策、服务说明 → 使用 company_info 工具
- **FAQ**：搜索常见问题解答 → 使用 faq_search 工具

## 工具使用规则
1. 用户问积分相关 → 必须使用 loyalty_points_lookup 工具
2. 用户要兑换积分 → 必须使用 loyalty_points_redeem 工具
3. 用户问物流相关 → 必须使用 shipment_tracking 工具
4. 用户问优惠券相关 → 必须使用 customer_coupon_lookup 工具
5. 用户要使用优惠券 → 必须使用 coupon_apply 工具
6. 用户要产品推荐 → 必须使用 product_recommendation 工具
7. 用户问反馈相关 → 使用 customer_feedback_lookup 工具
8. 用户要提交反馈 → 使用 customer_feedback_submit 工具
9. 用户问订单相关 → 必须使用 order_lookup 工具
...
class GeneralAgent(BaseAgent):
    ...
    def get_tools(self):
        return [
            ...
            customer_feedback_submit, loyalty_points_redeem, coupon_apply,
        ]
